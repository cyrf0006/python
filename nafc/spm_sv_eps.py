...
for fname in filelist:
    ...
    ## peps_list.append([p_eps[digitized == i].mean() for i in range(0, len(Pbin))])
    ## N2_list.append([N2[digitized == i].mean() for i in range(0, len(Pbin))])
    ## fluo_list.append([F[digitized == i].mean() for i in range(0, len(Pbin))])
    #date_list.append([pdtime[digitized == i].mean() for i in range(0, len(Pbin))])
...
# Epsilon to Sv for scatterplot comparision
...
#levels_sv = 10
c = ax0.contourf(df_sv.index, df_sv.columns, np.log10(df_sv.T), levels_sv, cmap=plt.cm.gray_r)
...
# Using Seaborn
df = pd.DataFrame({'eps':np.log10(df_eps2Sv.values.flatten()),'Sv125':10*np.log10(df_125_2_eps.values.flatten()),'Sv38':10*np.log10(df_38_2_eps.values.flatten())})
...
# --------  Another test VMP only (works well!) -- ##
