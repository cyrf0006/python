...
## ----  Load ice data ---- ##
df = pd.read_excel('/home/cyrf0006/AZMP/state_reports/ColbourneStuff/NL_SEA_ICE_EXTENT_DATA_1963_2018.xlsx', header=4)
...
df_winter = df[df.index.month<=3]
df_spring = df[(df.index.month>=4) & (df.index.month<=6)]

df_winter_annual = df_winter.resample('As').mean()
df_spring_annual = df_spring.resample('As').mean()
#df_annual = pd.concat((df_winter_annual,df_spring_annual), axis=1).mean(axis=1)
df_annual = df.resample('As').mean()

df_iroc = pd.concat((df_winter_annual,df_spring_annual,df_annual), axis=1)
